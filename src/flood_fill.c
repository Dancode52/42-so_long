...
/*   Updated: 2026/02/19 15:00:01 by dlanehar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	flood_fill(char **map, t_map_count *map_info, t_map_errors *error)
{
	...
	go_right_till_wall(map, row, column);
	printmap(map);
	go_left_till_wall(map, row, column);
	printmap(map);
	go_up_till_wall(map, row, column);
	printmap(map);
	go_down_till_wall(map, row, column);
	printmap(map);
}
