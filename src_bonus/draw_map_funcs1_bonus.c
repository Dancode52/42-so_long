...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   draw_map_funcs1_bonus.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: dlanehar <dlanehar@student.42angouleme.    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/01 11:24:38 by dlanehar          #+#    #+#             */
/*   Updated: 2026/03/02 12:57:51 by dlanehar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (row == 0)
	{
		if (column == 0)
			mlx_put_transformed_image_to_window(game->mlx, game->win,
				game->img_tlwall, column * game->tile_size_px,
				row * game->tile_size_px, 4, 4, 0);
		if (column == game->map_info.map_width - 1)
			mlx_put_transformed_image_to_window(game->mlx, game->win,
				game->img_trwall, column * game->tile_size_px,
				row * game->tile_size_px, 4, 4, 0);
	}
}
...
void	draw_b_l_r_walls(t_game_state *game, size_t row, size_t column)
{
	if (row == game->map_info.map_height - 1)
	{
		if (column == 0)
			mlx_put_transformed_image_to_window(game->mlx, game->win,
				game->img_blwall, column * game->tile_size_px,
				row * game->tile_size_px, 4, 4, 0);
		if (column == game->map_info.map_width - 1)
			mlx_put_transformed_image_to_window(game->mlx, game->win,
				game->img_brwall, column * game->tile_size_px,
				row * game->tile_size_px, 4, 4, 0);
	}
}
...
{
	if (row > 0 && row < game->map_info.map_height - 1)
	{
		if (column == 0)
			mlx_put_transformed_image_to_window(game->mlx, game->win,
				game->img_lwall, column * game->tile_size_px,
				row * game->tile_size_px, 4, 4, 0);
		if (column == game->map_info.map_width - 1)
			mlx_put_transformed_image_to_window(game->mlx, game->win,
				game->img_rwall, column * game->tile_size_px,
				row * game->tile_size_px, 4, 4, 0);
	}
}
...
void	draw_t_b_walls(t_game_state *game, size_t row, size_t column)
{
	if (column > 0 && column < game->map_info.map_width - 1)
	{
		if (row == 0)
			mlx_put_transformed_image_to_window(game->mlx, game->win,
				game->img_twall, column * game->tile_size_px,
				row * game->tile_size_px, 4, 4, 0);
		if (row == game->map_info.map_height - 1)
			mlx_put_transformed_image_to_window(game->mlx, game->win,
				game->img_bwall, column * game->tile_size_px,
				row * game->tile_size_px, 4, 4, 0);
	}
}
